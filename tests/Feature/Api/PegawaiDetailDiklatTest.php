...
class PegawaiDetailDiklatTest extends TestCase
{
    ...
    public function test_pegawai_detail_includes_complete_diklat_fields(): void
    {
        ...
        $response = $this->getJson("/api/pegawai/{$pegawai->id}", [
            'Authorization' => "Bearer {$token}",
        ]);

        $response->assertOk()
            ->assertJsonPath('data.diklat.0.waktu', '08:30:00')
            ->assertJsonPath('data.diklat.0.created_by', 'Admin SIMPEG')
            ->assertJsonPath('data.diklat.0.total_biaya', '1500000.00')
            ->assertJsonPath('data.diklat.0.jenis_biaya', 'APBD')
            ->assertJsonPath('data.diklat.0.jenis_pelaksana', 'internal')
            ->assertJsonPath('data.diklat.0.catatan', 'Wajib membawa laptop.')
            ->assertJsonPath('data.diklat.0.sertif', 'dokumen/sertif-diklat/sertif-test.pdf')
            ->assertJsonPath('data.diklat.0.no_sertif', 'CERT-001');
    }
