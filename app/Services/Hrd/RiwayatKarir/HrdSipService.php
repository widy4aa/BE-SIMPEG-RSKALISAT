...
class HrdSipService extends BaseHrdRiwayatKarirService
{
    ...
    public function updateSip(int $id, int $pegawaiId, array $payload, ?UploadedFile $file = null): array
    {
        ...
        $data = [];
        if (array_key_exists('jenis_sip_id', $payload)) {
            $data['jenis_sip_id'] = $payload['jenis_sip_id'];
        }
        if (array_key_exists('nomor_sip', $payload)) {
            $data['nomor_sip'] = $payload['nomor_sip'];
        }
        if (array_key_exists('tanggal_terbit', $payload)) {
            $data['tanggal_terbit'] = $payload['tanggal_terbit'];
        }
        if (array_key_exists('tanggal_kadaluarsa', $payload)) {
            $data['tanggal_kadaluarsa'] = $payload['tanggal_kadaluarsa'];
        }
