...
class HrdJabatanService extends BaseHrdRiwayatKarirService
{
    ...
    public function updateJabatan(int $id, int $pegawaiId, array $payload, ?UploadedFile $file = null): array
    {
        ...
        $jabatanData = [];
        if (array_key_exists('unit_kerja_id', $payload)) {
            $jabatanData['unit_kerja_id'] = $payload['unit_kerja_id'];
        }
        if (array_key_exists('nama_jabatan', $payload)) {
            $jabatanData['nama'] = (string) $payload['nama_jabatan'];
        }
        if (array_key_exists('tmt_mulai', $payload)) {
            $jabatanData['tmt_mulai'] = $payload['tmt_mulai'];
        }
        if (array_key_exists('tmt_selesai', $payload)) {
            $jabatanData['tmt_selesai'] = $payload['tmt_selesai'];
        }

        $pivotData = [];
        if (array_key_exists('is_current', $payload)) {
            $pivotData['is_current'] = (bool) $payload['is_current'];
        }
        if (array_key_exists('tmt_mulai', $payload)) {
            $pivotData['started_at'] = $payload['tmt_mulai'];
        }
        if (array_key_exists('tmt_selesai', $payload)) {
            $pivotData['ended_at'] = $payload['tmt_selesai'];
        }
        if (array_key_exists('note', $payload)) {
            $pivotData['note'] = $payload['note'];
        }
